...
module Blacklight::PrimoCentral
  class SearchBuilder < Blacklight::SearchBuilder
    ...
    def skip_search?(params)
      ...
      bl8_advance_form_blank = search_state.params[:clause]&.map { |_, q| q[:query] || q["query"] }&.all? { |v| v.blank? }
      bl8_advance_form_blank = true if bl8_advance_form_blank.nil?

      params[:skip_search?] =  [ "q", "q_1", "q_2", "q_3"].all? do |key|
        next true if search_state.params[key].nil?
        ["*", "", nil].include?(search_state.params[key])
