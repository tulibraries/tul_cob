...
module RequestHelper
  ...
  def request_modal(mms_id, pickup_locations, request_level)
    link_to(t("requests.request_button"), "#", id: "request-btn-#{mms_id}", class: "btn btn-sm btn-primary request-button float-right #{params[:action] == 'index' ? 'search-results-request-btn' : 'record-page-request-btn'}", data: { "blacklight-modal": "trigger", "action": "availability#modal", "target": "availability.href" })
  end
