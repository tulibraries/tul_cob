...
class BlacklightAlma {
  ...
  * @param {string} id - The ID of the availability button.
  * @param {object} holding - The holding information.
  * @returns {string}
  */
  ...
  createIframeElement = function(url) {
    var iframe = $("<iframe>");
  ...
  formatHolding = function (holding) {
    if(holding['inventory_type'] == 'physical') {
      return this.availabilityInfo(holding);
    }
  };
  ...
  availableHoldings = function (holdings) {
    ...
    this.sortedLibraries(availHoldings);
  ...
  * @returns {string}
  */
  formatHoldings = function (holdings) {
  var html = ""
  var available = this.availableHoldings(holdings);
  var check = this.checkHoldings(holdings);
            ...
            if (baObj.availability[id]) {
                ...
                if (holdings.length > 0) {
                    var formatted = $.map(holdings, function(holding) {
                      baObj.availabilityButton(id, holding);
                      return baObj.formatHolding(holding);
                    });
                    return baObj.formatHoldings(formatted);
                } else {
                  baObj.noHoldingsAvailabilityButton(id);
                }
            }
        }).join("<br/>");
  ...
  * @param html
  */
  renderAvailability = function(element, html) {
  ...
  * Subclasses should override to customize.
  */
  errorLoadingAvailability = function (idList) {
  ...
  */
  showElementsOnAvailabilityLoad = function () {
  ...
  * @param attemptCount
  */
  loadAvailabilityAjax = function (idList, attemptCount) {
    var baObj = this;
    if(idList.length > 0) {
  ...
  */
  registerToggleAvailabilityDetails = function() {
    var baObj = this;
  ...
  */
  toggleAvailabilityDetailsForRecord = function(toggleElement, containerElement) {
    var baObj = this;
  ...
  */
  loadAvailability = function() {
    ...
    baObj.registerToggleAvailabilityDetails();

    var allIds = $(".availability-ajax-load").map(function (index, element) {
        return $(element).data("availabilityIds");
    }).get();

    var idArrays = baObj.partitionArray(baObj.BATCH_SIZE, allIds);
    ...
    idArrays.forEach(function(idArray) {
        ...
        .then(id =>{ baObj.waitForRequestUrlData(id)
        .then(id => { baObj.clickRequestButton(id) })})
    });
  ...
  /**
    ...
    */
  waitForRequestUrlData(id) {
    return this.waitForElementById("request-url-data-" + id)
    .then(_ => { return id })
  }
  /**
  ...
  */
  checkAndPopulateMissing = function() {
    var baObj = this;
    ...
    $(".availability-ajax-load").filter(function(index, element) {
      ...
      baObj.noHoldingsAvailabilityButton($(element).data("availabilityIds"));
