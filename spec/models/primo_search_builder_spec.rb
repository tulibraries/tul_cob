...
RSpec.describe Blacklight::PrimoCentral::SearchBuilder , type: :model do
  ...
  describe ".process_advanced_search" do
    ...
    context "legacy advanced search with multiple rows" do
      let(:params) do
      ...
      it "maps fields and preserves both populated rows" do
        expected = [
          { "value" => "foo", "field" => :title, "precision" => nil, "operator" => "OR" },
          { "value" => "bar", "field" => :creator, "precision" => nil, "operator" => nil }
        ]
        expect(primo_central_parameters["query"]["q"]["value"]).to eq(expected)
      end
    end
