...
describe('AvailabilityController', () => {
  (function(){
  	  $.fn.longList = function() {
  	    ...
  	    return this.each(function(){
  	      ...
  	      $more = $('<button class="btn btn-sm bg-lighter-grey text-cherry-red border border-lighter-grey m-2 show-all">Show All<span class="sr-only"> at ' + type + '</span></button>'),
  	      $less = $('<button class="btn btn-sm bg-lighter-grey text-cherry-red border border-lighter-grey m-2 show-less">Show Less<span class="sr-only"> at ' + type + '</span></button>');
  ...
  const controller = `
    <div data-controller="availability" data-availability-url="http://localhost:32770/almaws/item/991030169919703811?redirect_to=http%3A%2F%2Flocalhost%3A32770%2F%3Ff%255Bavailability_facet%255D%255B%255D%3DAt%2Bthe%2BLibrary">
      <div class="controls">
        <button data-action="availability#item" data-availability-ids="991030169919703811" class="btn btn-sm btn-default availability-toggle-details" data-toggle="collapse" data-target="#physical-document-1" data-availability-target="button" id="available_button-1">
          <i class="fa fa-spinner" role="spinbutton"></i>
          <span>Loading...</span>
        </button>
        <div data-availability-target="spinner" class="spinner">
          <i class="fa fa-spinner" role="spinbutton"></i>
          <span>Loading Availability</span>
        </div>

        <div id="requests-container-991030169919703811" class="hidden requests-container" data-controller="requests" data-availability-target="request" data-show-target="request">
      ...
      <div data-availability-ids="991030169919703811" id="physical-document-1" class="collapse panel-content avail-container index-avail-container">
        <div class="table-wrapper hidden">
          <div data-availability-target="panel"></div>
          <div></div>
        </div>
      </div>
    </div>
  ...
  describe('#item', () => {
    ...
    test('repsone text is added to panel target', () => {
      const response = $('div[data-availability-target="panel"]').text()
    ...
    test('panel target unhidden', () => {
      let hidden = $('div[data-availability-target="panel"]')
