...
RSpec.configure do |config|
  ...
  config.before(:each) do
    ...
    stub_request(:get, /.*\.exlibrisgroup\.com\/almaws\/v1\/bibs\/japan_only\/holdings\/.*\/items/).
    to_return(status: 200,
              headers: { "Content-Type" => "application/json" },
              body: File.open(SPEC_ROOT + "/fixtures/requests/japan_only.json"))

    stub_request(:get, /.*\.exlibrisgroup\.com\/almaws\/v1\/bibs\/japan_with_multiple_libraries\/holdings\/.*\/items/).
      to_return(status: 200,
                headers: { "Content-Type" => "application/json" },
