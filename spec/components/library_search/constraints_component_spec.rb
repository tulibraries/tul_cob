...
RSpec.describe LibrarySearch::ConstraintsComponent, type: :component do
  ...
  describe "#initialize" do
    it "uses the legacy constraint layout for query constraints" do
      ...
      expect(cmp.instance_variable_get(:@query_constraint_component)).to eq(LibrarySearch::ConstraintLayoutComponent)
    end

    it "honors an explicit nil start-over component" do
      config = Blacklight::Configuration.new
      real_state = Blacklight::SearchState.new({}, config)
      cmp = described_class.new(search_state: real_state, start_over_component: nil)

      expect(cmp.instance_variable_get(:@start_over_component)).to be_nil
    end
  end
  ...
  describe "#constraint_presenters" do
    ...
    it "skips lc_outer_facet presenters when lc_inner_facet is selected" do
      ...
      expect { |b| component.send(:constraint_presenters).each(&b) }.to yield_successive_args(inner_presenter)
    end
    ...
    it "skips library facets when a matching location is selected" do
      ...
      expect { |b| component.send(:constraint_presenters).each(&b) }.to yield_successive_args(location_presenter)
    end
